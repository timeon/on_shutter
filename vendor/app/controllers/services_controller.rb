class ServicesController < ApplicationController
  ...
  def newaccount
    if params[:commit] == "Cancel"
      ...
      @newuser.login=  session[:authhash][:email]
      @newuser.password=  "password"
      @newuser.services.build(:provider => session[:authhash][:provider], :uid => session[:authhash][:uid], :name => session[:authhash][:name], :email => session[:authhash][:email])
  ...
  def create
    ...
    if omniauth and params[:service]
      ...
      if service_route == 'facebook'
        @authhash[:email]   = omniauth['extra']['raw_info']['email'] || ''
        @authhash[:name]    = omniauth['extra']['raw_info']['name'] || ''
        ...
        @authhash[:name]    = omniauth[:info]['name'] || ''
        @authhash[:uid]     = omniauth['uid'].to_s || ''

        @authhash[:provider]= omniauth['provider'] || ''
        if service_route == 'twitter'
          @authhash[:email]  = @authhash[:uid] + "@twitter.user"
        end
      else        
      ...
      if @authhash[:uid] != '' and @authhash[:provider] != ''
        ...
        if user_signed_in?
          if @auth
            ...
            current_user.services.create!(:provider => @authhash[:provider], :uid => @authhash[:uid], :name => @authhash[:name], :email => @authhash[:email])
  ...
  def get_auth
    ...
    if !@auth and @authhash[:email] and @authhash[:email].length > 4
      @auth = Service.find_by_email(@authhash[:email])
      
      if !@auth
        user = User.find_by_email(@authhash[:email])
        if user
          @authhash[:user_id] = user.id
          @auth = Service.new(@authhash) 
          @auth.save                             
        end
      end
    end
